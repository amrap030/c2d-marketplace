...
if [ ! -f "$SUMS" ] 
then
	echo "proof,timestamp,deployment_gas,compiled_size,proving_key_size,verification_key_size,verifier_size" > $SUMS
fi
...
do
    ...
    verifier_size=$(du -kh $DIR/$proof/zokrates/verifier.sol | cut -f1 | sed 's/\([0-9]\),/\1./g' | xargs)
