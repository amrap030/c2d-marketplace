...
Now you should access CLI script: $ crypto-service --help
Alternatively you can access CLI script via poetry run: $ poetry run crypto-service --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_CRYPTO_SERVICE_COMPLETE=source_bash crypto-service)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_CRYPTO_SERVICE_COMPLETE=source_zsh crypto-service)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_CRYPTO_SERVICE_COMPLETE=source_fish crypto-service)' >> ~/.config/fish/completions/crypto-service.fish
EOF