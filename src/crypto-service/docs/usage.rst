...
.. code-block:: bash
    ...
    $ crypto-service --help
    Usage: crypto-service [OPTIONS] COMMAND [ARGS]...

      Crypto-service CLI root.
    ...
    Commands:
      serve  crypto-service CLI serve command.
    $ crypto-service serve --help
    Usage: crypto-service serve [OPTIONS]
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name crypto-service -p 8000:8000 crypto-service:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/crypto-service:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  crypto-service
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
You can implement your own web routes logic straight away in ``crypto_service.controllers`` submodule. For more information please see `FastAPI documentation <https://fastapi.tiangolo.com/tutorial/>`__.
...
.. code-block:: bash
    ...
    Usage:
      ...
      clean-image      Clean crypto-service image
      install          Install crypto-service with poetry
      metrics          Run crypto-service metrics checks
      unit-test        Run crypto-service unit tests
      integration-test  Run crypto-service integration tests
      docs             Build crypto-service documentation
...
* RedisClient ``crypto_service.app.utils.redis``
* AiohttpClient ``crypto_service.app.utils.aiohttp_client``
...
.. code-block:: python
    ...
    from crypto_service.app.utils import RedisClient
...
.. literalinclude:: ../crypto_service/app/exceptions/http.py
    :language: python
...
All application configuration is available in ``crypto_service.config`` submodule.
...
.. literalinclude:: ../crypto_service/config/gunicorn.py
    :language: python

Routes
~~~~~~

Endpoints are defined in ``crypto_service.app.router`` submodule. Just simply import your controller and include it to FastAPI router:

.. literalinclude:: ../crypto_service/app/router.py
    :language: python
